...
export class CoreRunner {
  static async executeTurn(options: TurnOptions): Promise<TurnResult> {
    ...
    // 4. Adaptive 1-by-1 Intake Interview for CONOPS if not yet completed
    ...
    if (specialist.id === 'conops' && !hasApprovedConops && !state.intake.completed) {
      ...
      // Dynamic Adaptive Questions 3, 4, 5 generated by LLM based on user's previous responses
      if (qIndex >= 2 && qIndex < 5) {
        const answerKey = `question_${qIndex}_answer`;
        stateManager.recordIntakeAnswer(answerKey, userPrompt);
        stateManager.incrementIntakeQuestion();

        onStatusUpdate?.('Formulating next guided intake question...');
        const intakePrompt = `You are the CONOPS Specialist conducting an adaptive intake interview for a physical/hardware engineering project.
Review the user's project details gathered so far:
${JSON.stringify(stateManager.getState().intake.answers, null, 2)}

User's latest response: "${userPrompt}"

Current Question Count: ${qIndex + 1} of 5.
...
- Operating Location & Climate (indoor/outdoor, geographic climate extremes like Florida heat/humidity vs cold/snow, terrain).
...
Acknowledge their previous answer briefly, then ask the next question clearly.
Respond with ONLY the response text to the user.`;
        ...
        try {
          ...
          const nextQuestionText = intakeRes.content || `Thank you. Next question:\n\n${qIndex + 1}. What is the operating location, climate conditions, and environmental environment for this project?`;
          ...
          // fallback
          const fallbackMsg = `Thank you. Next guided question:\n\n${qIndex + 1}. Where will this project be operated (indoor/outdoor, climate conditions, terrain), and what are the primary use cases?`;
          return {
            specialistId: 'conops',
            specialistName: 'CONOPS Intake',
            output: fallbackMsg,
      ...
      // If at question 5+, record final answer and mark intake complete so we synthesize the grounded draft
